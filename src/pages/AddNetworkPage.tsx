...
const NetworkForm = ({ onSuccess, masterData, network, mode = "create" }: NetworkFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "request") {
        ...
        result = await supabase.from("network_requests").insert([
          {
            ...networkData,
      ...
      toast({
        ...
        description:
          mode === "request"
            ? "Request sent successfully. Admin will review."
            : `Network ${network ? "updated" : "created"} successfully`,
      });
  ...
  // Bulk file upload (still inserts directly to networks)
...
/**
 * Page wrapper — THIS is the default export from /pages/AddNetworkPage.tsx.
 * Users submit requests, so we pass mode="request".
 */
const AddNetworkPage = () => {
  return (
    <div className="container py-8">
      <NetworkForm onSuccess={() => {}} masterData={null} mode="request" />
    </div>
  );
};
