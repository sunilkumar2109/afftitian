...
// Put this near the top if you want to type the event (optional):
// import React, { useState, useEffect } from "react";
// (Your file already imports useState/useEffect — adding React is optional for types)

/////////////////////
// BannerDisplay: opens all links in link_urls in separate tabs
/////////////////////
const BannerDisplay = ({
  banners,
  section,
  ...
  const handleClick = (e: any) => {
    ...
    // collect links (prefer link_urls array, fallback to single link_url)
    const links: string[] = currentBanner.link_urls?.length
      ? currentBanner.link_urls
    ...
    // open each link in a new tab, stagger by small delay to reduce popup-block chances
    links.forEach((link, idx) => {
      ...
      if (idx === 0) openFn();
      else setTimeout(openFn, idx * 75); // 75ms stagger
    });
  };

  return (
    <div className={`${containerClass} cursor-pointer`} onClick={handleClick}>
      <img
        src={bannerSrc}
        alt={`${section} banner`}
...
/////////////////////
// SidebarBannerDisplay: clicks open all links in the banner's link_urls
/////////////////////
    ...
    <div className="space-y-4">
      {banners.map((banner) => {
        ...
        const handleClick = (e: any) => {
          ...
          const links: string[] = banner.link_urls?.length
            ? banner.link_urls
            : banner.link_url
            ? [banner.link_url]
            : [];
...
const Browse = () => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto m-12 px-4 bg-gray-900 bg-opacity-95 rounded-2xl shadow-lg">
